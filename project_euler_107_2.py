...
class Graph:
	...
	def weight(self):
		w = math.fsum(self.distances.values())
		return w
...
def dijsktra(graph, initial):
	...
	while nodes: 
		...
		for edge in graph.edges[min_node]:
			weight = current_weight + graph.distances[(min(edge, min_node),max(edge, min_node))]
	...
	new_path = []
	for k in path.keys():
		new_path.append((k,path[k]))
	return new_path

verts = """-,-,-,427,668,495,377,678,-,177,-,-,870,-,869,624,300,609,131,-,251,-,-,-,856,221,514,-,591,762,182,56,-,884,412,273,636,-,-,774
...
matrix = [v.split(',') for v in verts.split('\n')]
ppVerts(matrix)
...
edges = makePairs(matrix)

old_graph = Graph()
for  i in range(40):
	old_graph.add_node(i)
for a in edges:
	old_graph.add_edge(a[0],a[1],float(matrix[a[0]][a[1]]))

old_weight = old_graph.weight()

actual_path = []
actual_weight = old_weight
print actual_weight

for i in range(40):
	new_path = dijsktra(old_graph,i)
	new_graph = Graph()
	[new_graph.add_node(a) for a in range(40)]
	[new_graph.add_edge(a[0],a[1],float(matrix[a[0]][a[1]])) for a in new_path]
	weight = new_graph.weight()
	if weight < actual_weight:
		actual_weight = weight
		actual_path = new_path


print 'Solution: {}'.format(old_weight-actual_weight)

G = nx.Graph()
G.add_nodes_from(range(40))
G.add_edges_from(new_path)
