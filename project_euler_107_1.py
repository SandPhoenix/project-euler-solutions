...
verts = [v.split(',') for v in verts.split('\n')]
...
def makePairs(v):
	...
	for a in range(nodes_n):
		for b in range(nodes_n):
			if v[a][b] != '-':
				pairs.append(tuple(set([a,b])))
	return list(set(pairs))


ppVerts(verts)
edges = makePairs(verts)
print len(edges)
...
class Graph:
	...
	def weight(self):
		return math.fsum(self.distances.values())
...
def dijsktra(graph, initial):
	...
	return visited, new_path

def checkWeight(v,path):
	weight = 0
	for pair in path:
		weight += int(v[pair[0]][pair[1]])
	return weight
...
for a in range(40):
	for b in range(40):
		if verts[a][b] != '-':
			graph.add_edge(a,b,int(verts[a][b]))
w = []
for i in range(40):
	path = dijsktra(graph,i)[1]
	x = checkWeight(verts,path)
	print graph.weight()
	w.append(x)

new_w =  min(w)
orig_w = checkWeight(verts,edges)
print 'save: {}'.format(orig_w-new_w)


G.add_edges_from(dijsktra(graph,w.index(new_w))[1])
